...
logging_file_name = abspath(join('.', 'log', log_file_name_for_diff_endpoint[1], __name__))
logger = Logger(log_name=logging_file_name+'.log', log_level='1', logger_name=log_file_name_for_diff_endpoint[1]).get_log

# define adb logcat output redirected into which file
...
def feature_test_procedure(content_name, dap_status, dap_profile, dap_feature_type, dap_feature_value, expected_str=''):
    ...
    register_logger_name(log_file_name_for_diff_endpoint[1])
    ...
    log_file_name = logFileLocation + log_file_name_for_diff_endpoint[1] + '/' + current_method_name + '_' + \
...
def test_log_tc78_bass(content_name, dap_status, dap_profile, dap_feature_type, dap_feature_value, expected_str=''):
    ...
    em1 = LogComparison(log_file_name_for_diff_endpoint[1])
    ...
    if base_enhancer_value == '1':
        logger.info("get bass1 enhancer value is 1 .")
        pass
    else:
        assert 0
        pass
    if virtual_base_value == '1':
        logger.info("get virtual1 bass value is 1 .")
        pass
    else:
        assert 0
        pass
