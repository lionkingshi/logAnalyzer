...
# blue tooth's behavior is same as 3.5mm headphone's in current DAX3.5 release
# copy all test cases to here from file test_case_for_35mm_headphone.py
...
def test_log_dynamic_profile_default_value_verify(content_name, content_type,
    ...
    assert_specified_profile_default_values_result(
        profile_name[0],
        tuning_endpoint_name[2],  # modify this parameter to match headphone endpoint
        endpoint_type_in_module,
        content_type)
...
def test_log_movie_profile_default_value_verify(content_name, content_type,
                                                dap_status, dap_profile, dap_feature_type,
                                                dap_feature_value):
    """
    Test Case ID    : TC-new
...
def test_log_custom_profile_default_value_verify_for(content_name, content_type,
                                                     dap_status, dap_profile, dap_feature_type,
                                                     dap_feature_value):
    """
    Test Case ID    : TC-new
    ...
    """
    caller_name = test_log_custom_profile_default_value_verify_for.__name__
    specified_profile_default_value_test_procedure_dax3(
        caller_name,
        endpoint_type_in_module,
        content_name,
...
# for blue tooth endpoint , the reference level is applied
@pytest.mark.parametrize('content_name,content_type,dap_status,dap_profile,dap_feature_type,dap_feature_value',
