...
class TuningFileParser:
    ...
    def get_feature_aobs_arbs(self, dap_feature, endpoint):
        ...
        try:
            ...
            print (e)
    #get geq bands
    def get_geq_bands(self,profile):
        profile_patten=".//profile[@name='%s']" % profile
        profile=self.__root.find(profile_patten)
        profile_id=profile.get("id")
        preset_id=profile_id
        preset_patten=".//preset[@id='%s']" % preset_id
        e_preset=self.__root.find(preset_patten)
        #print "e_ieq_preset.tag "+e_ieq_preset.get("id")
        e_bands=e_preset.findall(".//"+"band_geq")
        return e_bands
    ...
    def set_feature_tuning(self, dap_feature, feature_value, endpoint, ao_or_ar=""):
        ...
        try:
            ...
            print (e)
    #set value for gebs
    def set_feature_gebs(self,feature_value,profile):
        e_geg_bands=self.get_geq_bands(profile)
        gains_list=feature_value.split(",")
        i=0
        for band in e_geg_bands:
            band.set("gain",gains_list[i])
            i=i+1
    ...
    def set_feature_profile(self, dap_feature, feature_value, profile, endpoint_type=""):
        try:
            if dap_feature=="gebs":
                self.set_feature_gebs(feature_value,profile)
                print "Set gebs succeed for {0} profile to value {1}".format(profile,feature_value)
                return 
