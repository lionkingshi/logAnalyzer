...
def crate_a_new_profile_in_sequence(_xml_real_path, _new_profile_num):
    assert isinstance(_new_profile_num, int)
    assert _new_profile_num > 0, "the number of added profile should be bigger than 0!"
    _lang = xml.parse(_xml_real_path)
    _root = _lang.getroot()
    ...
    save_xml_file(_lang, "dax-default-copy-in-sequence.xml")
...
def insert_profile_paras_copy_from_a_existing_profile(_root, _copied_profile_name, current_added_profile_index):
    ...
    _new_id_profile.attrib['name'] = str(_copied_profile_name) + str(current_added_profile_index)
...
if __name__ == "__main__":
    ...
    crate_a_new_profile_in_sequence(filename2, 12)
