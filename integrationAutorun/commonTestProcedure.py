...
def assert_dap_be_related_feature_result(_content_type, _be_value_dict):
...
def mi_on_test_procedure(caller_name, endpoint_id, content_name, content_type, dap_status, dap_profile):
    ...
    assert_dap_mi_and_vl_related_feature_result(content_type, mi_on_expected_dictionary, ref_lvl_expected_dictionary)
...
def no_log_print_when_dap_off_test_procedure(caller_name, endpoint_id, content_name, content_type):
    ...
    __generate_and_parse_log_file(caller_name, endpoint_id, content_name)
    assert_no_log_print_when_dap_off()
...
def assert_no_log_print_when_dap_off():
    result = get_result_no_log_exist_when_dap_off
    assert result, "error!!! Found log relating to dap paras when dap off"


def assert_decoding_drc_mode_related_feature_result(_content_type):
    pass


def assert_decoding_joc_down_mix_related_feature_result(_content_type, _down_mix=None):
...
def assert_dap_mi_and_vl_related_feature_result(_content_type, _mi_expected_value, _vl_expected_value):
    # verify mi related value is expected
    assert_dap_mi_related_feature_result(_content_type, _mi_expected_value)

    # verify volume level will always turn on for dolby content
    assert_dap_reference_level_related_feature_result(_content_type, _vl_expected_value)


def assert_dap_mi_related_feature_result(_content_type, _mi_expected_value):
    # verify no dap double processing
    __assert_no_double_processing_by_content_type(_content_type)

    # verify mi related value is expected
    __comparison_result(_content_type, _mi_expected_value)
...
def assert_dap_vl_related_feature_result(_content_type, _vl_expected_value):
    # verify no dap double processing
    __assert_no_double_processing_by_content_type(_content_type)
...
def assert_dap_reference_level_related_feature_result(_content_type, _ref_lvl_expected_dict):
    __assert_no_double_processing_by_content_type(_content_type)
    ...
    if _content_type in content_type_dolby:
        if 'vmb' in _ref_lvl_expected_dict.keys():
            ...
            __comparison_vmb_result(_vmb_tmp_value)
            _temp_dict['vmb'] = '0'
            __comparison_result(_content_type, _temp_dict)
            _temp_dict['vmb'] = _vmb_tmp_value
        else:
            __comparison_result(_content_type, _ref_lvl_expected_dict)
            pass
    else:
        pass


def assert_dap_dom_related_feature_result(_content_type, _dap_dom):
    __assert_no_double_processing_by_content_type(_content_type)
    __comparison_result(_content_type, _dap_dom)
...
def __comparison_vmb_result(_vmb_expected_value):
